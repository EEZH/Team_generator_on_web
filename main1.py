...
from functions import team_power, delta_power, team_clbr, players, year, players_photo_list, all_players_create, \
    db_list_getter
import json
...
def authorization():
    ...
    if request.method == "GET":
        if session.get('player'):
            return render_template("home.html", user_name=session["player"], menu_items=MENU_WITH_SESSION_ADMIN, year=year)
        return render_template("form_authorization.html", menu_items=MENU_WITHOUT_SESSION, year=year)
